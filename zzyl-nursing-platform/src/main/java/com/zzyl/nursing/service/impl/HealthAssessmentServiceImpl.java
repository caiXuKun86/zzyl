...
public class HealthAssessmentServiceImpl extends ServiceImpl<HealthAssessmentMapper, HealthAssessment> implements IHealthAssessmentService {
    ...
    public Long insertHealthAssessment(HealthAssessment healthAssessment) {
        String prompt = getContent(healthAssessment);

        // 2.调用百度千帆大模型，分析体检报告，获取分析结果
        String qianfanResult = aiModelInvoker.aiInvoker(prompt);
    ...
    private String getContent(HealthAssessment healthAssessment) {
        String result = (String) redisTemplate.opsForHash().get("healthReport", healthAssessment.getIdCard());
        if (result == null) {
            throw new BaseException("文件异常,请重新上传");
        }
        redisTemplate.opsForHash().delete("healthReport", healthAssessment.getIdCard());
        String content = "请以一个专业医生的视角来分析这份体检报告，报告中包含了一些异常数据，我需要您对这些数据进行解读，并给出相应的健康建议。\n" +
                "体检内容如下：\n" +
                result + "\n" +
                "\n" +
                "要求：\n" +
